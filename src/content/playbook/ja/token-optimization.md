...
データは **エージェントが直接読める形式** に置きましょう。`.xlsx` / `.docx` / `.pdf` を渡すたびに、エージェントは「パーススクリプトを書く → 実行 → 散らかった出力をコンテキストに読み戻す」という **3 ステップの迂回路** を強いられます。各ステップが余分なツール呼び出しで、パース後のテキストは同じ情報の Markdown 版より **3〜10 倍長い** のが普通 ── レイアウト情報（セル枠線・フォント・ページ区切り）がノイズとして混ざるからです。

ナレッジベース・仕様書・参照テーブルは **「人間とエージェントの両方が読む成果物」として扱い**、ソースを Markdown / CSV / プレーンテキストで保存しましょう。バイナリ原本は「アーカイブ」として残し、エージェントには **派生したテキスト版** を見せます。

| 元のフォーマット | 推奨される変換先 | なぜ |
| --- | --- | --- |
| 📊 `.xlsx` / Google Sheets | **CSV** または Markdown テーブル | カンマ区切りなら 1 セル = 数トークン |
| 📝 `.docx` / `.pptx` | **`.md`** | 見出し・リスト・コードブロックがそのまま構造になる |
| 📄 `.pdf` | **`.md` または `.txt`**（`pandoc` / `pdftotext` で変換） | レイアウト情報が剥がれて純データだけ残る |
| 🌐 Web ページ | **Markdown 抽出**（例: `r.jina.ai/<url>`） | 広告・ナビ・スクリプトが落ちる |
| 🖼️ テキストの画像 | **OCR → Markdown** | 画像は OCR ツール呼び出しが固定コスト |

> 💡 **「一度変換、何度も照会」** ── 元データが頻繁に更新されるなら、GitHub Actions やプリコミットフックで変換を自動化しましょう。`docs-source/*.xlsx` → `docs-agent/*.md` のようにツリーを分ければ、エージェント向け context を **決定的なビルド成果物** として扱えます。
