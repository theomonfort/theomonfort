...
今日、PR を開くのは **人間**・**Cloud Agent**・**Dependabot**・**エージェント型ワークフロー** と多様になった。差分の上に会話・チェック・自動分析が集まり、マージ前に品質を担保する場。
...
| 🛡️ Require status checks to pass | **test ＋ CodeQL** を必須 ＋ **Require branches to be up to date before merging** | CI とセキュリティスキャンが緑、かつ最新の main に対して検証されたときだけマージ |
