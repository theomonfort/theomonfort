...
- 🧠 `permissions.model` — 新規会話の既定モデル。`"auto"` で自動選択をデフォルトに（個別変更は可）
- 🚫 `permissions.disableBypassPermissionsMode` — `"disable"` で YOLO / バイパスモードを禁止（CLI `--yolo`・`/yolo`、VS Code の auto-approve）
- 🏪 `extraKnownMarketplaces` / `strictKnownMarketplaces`（トップレベル）— プラグイン marketplace を追加 / 限定
- 🧩 `enabledPlugins`（トップレベル）— 全ユーザーに自動インストールするプラグイン
