...
| 🦠 **Malware alerts** | 既知の悪意ある npm パッケージを検出 | GHAD に malware advisory が追加されあなたの依存と一致した時 | Security タブ(独立サブカテゴリ) |
...
> 🦠 **Malware alerting は opt-in**(`Settings → Code security → Dependabot → Malware alerts`)で、現在は **npm のみ**(OpenSSF Malware Streams 経由で他エコシステムも拡大予定)。2022 年に一度停止された機能だが、2026 年に独立サブカテゴリ + auto-triage rules でノイズを抑えて再ローンチ。
