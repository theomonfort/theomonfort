...
Copilot（Chat / CLI / Cloud Agent など）が `.github/` の設定ファイルや `package-lock.json` のような **触ってほしくないファイル** を勝手に書き換えないよう、**Hooks** でガードレールを張りましょう。特に次の章では CLI で複数のエージェントを並列に走らせるので、その前に安全網を用意しておくのが理想的です。
