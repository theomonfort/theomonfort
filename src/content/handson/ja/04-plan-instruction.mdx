...
duration: 15
summary: "このステップでは、Copilot 用の Instruction ファイル（Copilot に守らせるルールを書いた指示書）を作成します。"
---
...
**ポイント**: ファイルを**保存**すれば（コミットは不要）、以後の Copilot Chat の各リクエストに自動的に読み込まれます。応答が日本語に切り替わり、Stack 制約も自動で守られるようになります。詳しくは [VS Code: Use custom instructions](https://code.visualstudio.com/docs/agent-customization/custom-instructions) を参照。
