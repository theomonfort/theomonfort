...
**💡 ターミナルのヒント**: VS Code のターミナルでは **Ctrl+I** を押すと、コマンドラインや現在のプロジェクトについて Copilot に軽く質問できます。これは **Agent モードではありません**。既定のモデルも **Auto** なので、小さな質問をしても複数のエージェントセッションを立ち上げてしまう心配はありません。コンテキストウィンドウにはほとんど何も読み込まれないため、VS Code や CLI で開く通常のセッションよりもずっと安価です。詳しくは [プレイブック / トークン最適化](https://theomonfort.github.io/theomonfort/playbook/token-optimization?present=1&slide=11) を参照。
