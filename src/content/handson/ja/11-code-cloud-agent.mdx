...
### 11.1 — 既存の Issue に Cloud Agent をアサインする
...
**デモのおすすめ進め方**: まず **Issue 1（プレゼンモード本体）** と **Issue 4（Mermaid のバグ）** を同時にアサインしてみてください。両方が近いコードに触れるため **コンフリクトが起きやすく**、後述の **Fix with Copilot** でコンフリクト解決をそのまま体験できます。次に **Issue 7（目次サイドバーのデザイン試作）** もおすすめです。まず試作して見比べてから採用する進め方なら、各コンポーネントの仕上がりをしっかり詰められます。時間があれば、この「試作 → 比較 → 採用」を複数のコンポーネントで繰り返すと、結果が一段と良くなります。
