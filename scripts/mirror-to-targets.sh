...
#
# Usage:  ./scripts/mirror-to-targets.sh [--dry-run]
#
...
# Folders/files to EXCLUDE from mirroring (all targets)
COMMON_EXCLUDES=(
  "20-notes"
  "30-meetings"
  "README.md"
...
# Additional excludes per target (associative-style via function)
extra_excludes_for() {
  case "$1" in
    theomonfort/theomonfort) echo "50-security-checklists" ;;
  esac
}
...
sync_repo() {
  ...
  # Build per-target exclude list
  local EXCLUDES=("${COMMON_EXCLUDES[@]}")
  for extra in $(extra_excludes_for "$target_repo"); do
    EXCLUDES+=("$extra")
  done
